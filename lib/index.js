    ...
    compileTasks() {
        ...
        Object.keys(options.tasks).forEach(identifier => {
            ...
            var normalizedIdentifier =
                this.provider.naming.normalizeNameToAlphaNumericOnly(identifier)

            // consoleLog(override);
            if (!override.hasOwnProperty('role')) {
                // check if the default role can be assumed by ecs, if not, make it so
                if (
                    template.Resources.IamRoleLambdaExecution.Properties.AssumeRolePolicyDocument.Statement[0].Principal.Service.indexOf(
                        'ecs-tasks.amazonaws.com',
                    ) == -1
                ) {
                    template.Resources.IamRoleLambdaExecution.Properties.AssumeRolePolicyDocument.Statement[0].Principal.Service.push(
                        'ecs-tasks.amazonaws.com',
                    )

                    // check if there already is a ManagedPolicyArns array, if not, create it
                    if (
                        !template.Resources.IamRoleLambdaExecution.Properties.hasOwnProperty(
                            'ManagedPolicyArns',
                        )
                    ) {
                    ...
                    template.Resources.IamRoleLambdaExecution.Properties['ManagedPolicyArns'].push(
            ...
            if (options.tasks[identifier].hasOwnProperty('environment')) {
                ...
                target_environment = Object.assign(
                    target_environment,
                    options.tasks[identifier].environment,
                )
            ...
            var allowService = options.tasks[identifier]['allowService'] || true
            if (allowService) {
                    ...
                    Properties: Object.assign(
                        {
                            ...
                            TaskDefinition: {
                                'Fn::Sub': '${' + normalizedIdentifier + 'Task}',
                            },
